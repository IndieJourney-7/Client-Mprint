...
const Branded = () => {
  ...
  // Fetch branded products from API
  useEffect(() => {
    const fetchBrandedProducts = async () => {
      setLoading(true);
      try {
        const response = await fetch(`${API_BASE_URL}/products/branded?per_page=12`, {
          method: "GET",
          headers: {
        ...
        console.error("Failed to fetch branded products:", err);
        setProducts([]);
      } finally {
        setLoading(false);
      }
  ...
  const getProductImageUrl = (product) => {
    ...
    return "https://via.placeholder.com/200x170?text=Branded";
  };
