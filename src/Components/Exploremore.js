...
function Exploremore() {
  ...
  // Fetch featured products from API for "Explore More" section
  useEffect(() => {
    const fetchExploreProducts = async () => {
      setLoading(true);
      try {
        const response = await fetch(`${API_BASE_URL}/products/featured?per_page=12`, {
          method: "GET",
          headers: {
  ...
  const getProductImageUrl = (product) => {
    ...
    return "https://via.placeholder.com/200x170?text=Explore";
  };
